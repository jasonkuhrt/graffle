...
export const resolveOptions = (input: Input): Config => {
  ...
  return {
    ...
    libraryPaths: {
      client: input.libraryPaths?.client ?? `graphql-request/client`,
      scalars: input.libraryPaths?.scalars ?? `graphql-request/schema/scalars`,
      schema: input.libraryPaths?.schema ?? `graphql-request/schema`,
    },
    typeMapByKind,
    rootTypes: {
