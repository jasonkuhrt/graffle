...
import { encode } from '../../examples/__outputs__/extension|extension_opentelemetry.output.encoder.js'
import { runExample } from '../../scripts/generate-examples-derivatives/helpers.js'

test(`extension|extension_opentelemetry`, async () => {
  const exampleResult = await runExample(`./examples/extension|extension_opentelemetry.ts`)
  ...
  await expect(exampleResultMaybeEncoded).toMatchFileSnapshot(
    `../.././examples/__outputs__/extension|extension_opentelemetry.output.test.txt`,
  )
})
