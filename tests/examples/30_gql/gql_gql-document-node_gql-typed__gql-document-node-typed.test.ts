...
test(`gql_gql-document-node_gql-typed__gql-document-node-typed`, async () => {
  const exampleResult = await runExample(
    `./examples/30_gql/gql_gql-document-node_gql-typed__gql-document-node-typed.ts`,
  )
  ...
  await expect(exampleResultMaybeEncoded).toMatchFileSnapshot(
    `../../../examples/__outputs__/30_gql/gql_gql-document-node_gql-typed__gql-document-node-typed.output.txt`,
  )
})
