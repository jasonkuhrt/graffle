...
test(`output|output_return-error`, async () => {
  const exampleResult = await runExample(`./examples/output|output_return-error.ts`)
  ...
  await expect(exampleResultMaybeEncoded).toMatchFileSnapshot(
    `../.././examples/__outputs__/output|output_return-error.output.txt`,
  )
})
