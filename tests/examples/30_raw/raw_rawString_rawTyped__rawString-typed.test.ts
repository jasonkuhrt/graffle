...
test(`raw_rawString_rawTyped__rawString-typed`, async () => {
  const exampleResult = await runExample(`./examples/30_raw/raw_rawString_rawTyped__rawString-typed.ts`)
  ...
  await expect(exampleResultMaybeEncoded).toMatchFileSnapshot(
    `../../../examples/__outputs__/30_raw/raw_rawString_rawTyped__rawString-typed.output.txt`,
  )
})
