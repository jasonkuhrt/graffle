...
test(`extension_or-throw__or-throw`, async () => {
  const exampleResult = await runExample(`./examples/extension/extension_or-throw__or-throw.ts`)
  ...
  await expect(exampleResultMaybeEncoded).toMatchFileSnapshot(
    `../../../examples/__outputs__/extension/extension_or-throw__or-throw.output.txt`,
  )
})
