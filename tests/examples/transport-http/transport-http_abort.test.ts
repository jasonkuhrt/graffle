...
test(`transport-http_abort`, async () => {
  const exampleResult = await runExample(`./examples/transport-http/transport-http_abort.ts`)
  ...
  await expect(exampleResultMaybeEncoded).toMatchFileSnapshot(
    `../../../examples/__outputs__/transport-http/transport-http_abort.output.txt`,
  )
})
