...
test(`transport-http_headers_raw__headers`, async () => {
  const exampleResult = await runExample(`./examples/transport-http/transport-http_headers_raw__headers.ts`)
  ...
  await expect(exampleResultMaybeEncoded).toMatchFileSnapshot(
    `../../../examples/__outputs__/transport-http/transport-http_headers_raw__headers.output.txt`,
  )
})
